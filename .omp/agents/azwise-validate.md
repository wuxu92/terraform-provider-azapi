...
1. **Run the validation tool.** Call `azwise_validate` with no arguments (validates all knowledge files) or `resource=<name>` for a specific resource. The tool mechanically checks:
   ...
   - **Relational constraint paths**: each `RelationalRule.Paths` entry in `ConflictsWith`/`RequiredWith`/`ExactlyOneOf`/`AtLeastOneOf` must resolve through the SDK struct chain, same as other property paths (array-element `[*]` paths are skipped)
...
2. **Triage issues.** The tool returns issues with severity `error` (must fix) and `warning` (review needed). For each issue:
   ...
   - **`path_unresolved` warnings on relational fields** (ruleKind `ConflictsWith`/`RequiredWith`/`ExactlyOneOf`/`AtLeastOneOf`): a cross-property constraint path didn't resolve through the SDK models. Verify the ARM path in the SDK model files; if it legitimately targets an array element, a map key, or a sub-service API, it is out of scope — remove it from the knowledge file rather than forcing resolution.

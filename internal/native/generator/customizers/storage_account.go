...
//   - Several body fields carry generic AzureRM validators: UUID for the AD domain
//     GUID and resource-access-rule tenant IDs, and AzureResourceID for
//     resource-access-rule resource IDs.
func customizeStorageAccount(def *typegraph.ResourceDefinition) {
	...
	def.AddValidatorsFor("properties.networkAcls.ipRules.value", typegraph.Validator(storagevalidators.StorageAccountIPRule))

	// AD domain GUID is a UUID (AzureRM validation.IsUUID).
	def.AddValidatorsFor("properties.azureFilesIdentityBasedAuthentication.activeDirectoryProperties.domainGuid", typegraph.Validator(validators.UUID))
	...
	def.AddValidatorsFor("properties.networkAcls.resourceAccessRules.resourceId", typegraph.Validator(validators.AzureResourceID))
	def.AddValidatorsFor("properties.networkAcls.resourceAccessRules.tenantId", typegraph.Validator(validators.UUID))
}

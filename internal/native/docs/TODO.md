...
The declarative constraint engine **shipped**. `ConflictsWith / RequiredWith / ExactlyOneOf / AtLeastOneOf / AtMostOneOf` constraints are hand-declared per resource as `Hooks.Relational` in the resource's `<resource>_hooks.go` (`services.RelationalConstraint`), and enforced at runtime as reusable framework `ConfigValidators` over nested snake_case paths (`resource/configvalidators.go`). They are no longer synthesized into the generated descriptor — cross-field rules are runtime validation behavior, so they live with the other hand-owned hooks and stay out of `_gen.go`, tunable without regeneration. Mutual-exclusion members still drop `UseStateForUnknown` via the generator's `SuppressStateReuse` (independent of the constraint) so an omitted side clears instead of pinning stale state. Live examples: `azapi_virtual_network` `address_space` ↔ `ipam_pool_prefix_allocations` ExactlyOneOf; `azapi_storage_account` CMK RequiredWith.
